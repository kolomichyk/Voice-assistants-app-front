    ...
    const handleOnClickButtonAdd = async (id: number) => {
        ...
        const UpdateDraft = async () => {
            try {
                ...
                dispatch({ type: "UPDATE_DRAFT_ID", payload: result['draft_id'] })
            } catch (error) {
        ...
        console.log(`http://127.0.0.1:8000/actions/post/application/${id}/`)
        try {
            const response = await axios.post(`http://127.0.0.1:8000/actions/post/application/${id}/`, {}, {
                withCredentials: true
            });
        } catch (error) {
            console.error(error);
